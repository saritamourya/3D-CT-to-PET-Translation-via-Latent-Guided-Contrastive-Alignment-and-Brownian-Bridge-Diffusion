...
from configs.BBTest_options import BBTestOptions
...
# ==== Load Model ====
model = CT2PETDiffusionModel(opt.model_config, device).to(device)
checkpoint = torch.load(opt.diffusion_checkpoint, map_location=device)
print(f'diffusion checkpoint{opt.diffusion_checkpoint}')
...
if opt.use_ema:
    ema = EMA(model)
    ema.register()
    if os.path.exists(opt.ema_checkpoint):
        ema_weights = torch.load(opt.ema_checkpoint, map_location=device)
        ema.load_state_dict(ema_weights)
        ema.apply_shadow()
        print(f"[INFO] EMA weights applied from {opt.ema_checkpoint}")
    else:
        print(f"[WARNING] EMA checkpoint not found at: {opt.ema_checkpoint}. Using raw model weights.")
else:
    print("[INFO] Using raw model weights (EMA not enabled)")
...
def save_views(tensor, prefix, output_dir):
...
for i, data in enumerate(test_loader):
    ...
    for b in range(batch_size):
        A = data["A"][0].unsqueeze(0)
        B = data["B"][0].unsqueeze(0)  # Single volume
        path_string = data["A_paths"][0]
        shape = A.shape[2:]
        D, H, W = shape
        ...
        for center in tqdm(patches):
            ...
            patch_tensor_A = patch['A'].permute(0, 4, 1, 2, 3).to(device)
            patch_tensor_B = patch['B'].permute(0, 4, 1, 2, 3).to(device)
            batched_patches_A.append(patch_tensor_A)
            batched_patches_B.append(patch_tensor_B)
            batched_centers.append(center)
            if len(batched_patches_A) == 1024 or center == patches[-1]:
                input_tensor = torch.cat(batched_patches_A, dim=0)

                with torch.cuda.amp.autocast():
                    recon = model.sample(input_tensor)
                    outputs = recon.squeeze().cpu().numpy()

                for j, center in enumerate(batched_centers):
                    recon = outputs[j].squeeze() # (32, 32, 32)
                    cx, cy, cz = center
                    sx = sy = sz = overlap_dim
                    generated[cx-sx:cx+sx, cy-sy:cy+sy, cz-sz:cz+sz] += recon
                    overlap[cx-sx:cx+sx, cy-sy:cy+sy, cz-sz:cz+sz] += 1
                batched_patches_A.clear()
                batched_patches_B.clear()
                batched_centers.clear()
